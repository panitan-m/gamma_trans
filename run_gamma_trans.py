...
from models.ladder_net import LadderNetworkGamma
...
parser.add_argument('--freeze-bert', type=str,
...
for kf, (train_idx, test_idx) in enumerate(skf.split(x, y)):    
    ...
    model = LadderNetworkGamma(
        args, output_size=len(args.aspects),
    ...
    for epoch in range(args.epochs):
        with tqdm(unlabeled_loader) as p_bar:
            ...
            for batch_idx, (inputs_u, _) in enumerate(unlabeled_loader):
                ...
                writer.add_scalars("loss", {'train': losses.avg - den_losses.avg}, current_step)
